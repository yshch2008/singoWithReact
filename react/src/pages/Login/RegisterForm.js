...
class RegisterForm extends React.Component {
    ...
    onRegister = async (values) => {
        ...
        //加密密码
        const ciphertext =values.registerPassword// encrypt(values.registerPassword)
        const res = await post('/user/register', {
            Nickname:values.registerUsername,
            user_name:values.registerNickname,
            Password:ciphertext,
        ...
        return (
            <div>
                ...
                <Form hideRequiredMark>
                    ...
                    <Form.Item
                        help={<Promptbox info={getFieldError('registerNickname') && getFieldError('registerNickname')[0]} />}
                        style={{ marginBottom: 10 }}
                        wrapperCol={{ span: 20, pull: focusItem === 0 ? 1 : 0 }}
                        labelCol={{ span: 3, pull: focusItem === 0 ? 1 : 0 }}
                        label={<span className='iconfont icon-User' style={{ opacity: focusItem === 0 ? 1 : 0.6 }} />}
                        ...
                        {getFieldDecorator('registerNickname', {
                            ...
                            rules: [
                                ...
                                { pattern: /^[^\s']+$/, message: '不能输入特殊字符' },
                                { min: 3, message: '昵称至少为3位' },
                                { max: 30, message: '昵称不能长于30位' },
                            ]
                        })(
                            <Input
                                maxLength={16}
